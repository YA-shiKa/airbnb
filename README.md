...
- **The static map** uses OpenStreetMap's own free, keyless embed endpoint —
  ...
  flat image.
- **Messaging and identity verification are placeholders** ("Coming soon"),
  exactly as the assignment lists them under mocked/out-of-scope sections.
  Both are reachable from the navbar's user menu; messaging also has a
  "Contact host" entry point on every listing detail page. The entry points
  exist even though the features themselves don't yet.
